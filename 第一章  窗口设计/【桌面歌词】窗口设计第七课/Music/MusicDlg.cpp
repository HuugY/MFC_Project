...
BOOL CMusicDlg::OnInitDialog()
{
	...
	LoadBackSkin(AfxGetInstanceHandle(),TEXT("BACK_VIEW"));
	SetExtrude(false);
	...
	//创建歌词面板
	m_DesktoplyricDlg.Create(IDD_LYRISDLG,GetDesktopWindow());

	//模块路径
	TCHAR szWorkDirectory[MAX_PATH]=TEXT("");
	GetModuleFileName(AfxGetInstanceHandle(),szWorkDirectory,CountArray(szWorkDirectory));
	//分析文件
	for (INT i=lstrlen(szWorkDirectory);i>=0;i--)
	{
		if (szWorkDirectory[i]==TEXT('\\'))
		{
			szWorkDirectory[i]=0;
			break;
		}
	...
	//设置结果
	ASSERT(szWorkDirectory[0]!=0);
	strcat_s(szWorkDirectory,TEXT("\\杭娇-动人的传说.LRC"));
	
	m_DesktoplyricDlg.OpenLyric(szWorkDirectory);

	int nScreenWidth = GetSystemMetrics(SM_CXSCREEN);
	int nScreenHeight = GetSystemMetrics(SM_CYSCREEN);
	m_DesktoplyricDlg.SetWindowPos(&wndTopMost,nScreenWidth/20,nScreenHeight-130,(nScreenWidth-nScreenWidth/10),80,SWP_SHOWWINDOW);
